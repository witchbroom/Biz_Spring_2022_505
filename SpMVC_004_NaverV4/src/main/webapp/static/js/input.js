document.addEventListener("DOMContentLoaded", () => {
  ...
  // div.modal-result 에 포함된 요소 중에서 div.search-content를 가져오라
  ...
  modal_box?.addEventListener("click", (tb) => {
    ...
    if (td.tagName === "TD") {
      // td 를 감싸고있는 가장 가까운 영역의 tr을 추출하기
      const tr = td.closest("TR");
      // 추출된 tr의 속성 중에 data-isbn 으로 설정된 속성의 값을 추출하기
  ...
  // 데이터의 유효성 검사 - NOT NULL 으로 지정한 값이 입력되었는지 확인
