...
public class AddressServiceImplV1 implements AddressService {
	...
	/*
	 * 조건에 맞는 데이터를 가져와서
	 * pagination을 그리기 위한 SearchPage 클래스의 데이터를 만들 것
	 */
	@Override
	public void searchAndPage(Model model, SearchPage searchPage) {
		// pagination을 구현하기 위하여 전체 데이터 가져오기로 개수를 임시로 세팅
		searchPage.setOffset(0);
		searchPage.setLimit(addrDao.selectAll().size());
		
		// 1. 전체 데이터 개수가 몇개인지를 알아야 pagination을 구현할 수 있다
		// 화면 하단의 페이지 번호를 자동으로 계산하기 위함이다
		
		// 검색어가 없이 등장할 요청이 될 경우 searchPage 객체의 search 값이 null이 되어
		// 데이터가 전송되지 않음
		...
		// 만약 전체 데이터가 37개이고 한 페이지에 10개를 보여준다면
		// finalPageNo는 3이 될 것이다(정수형이기 때문에)
		long finalPageNo = totalCount / LIST_PER_PAGE;
		// 화면하단의 페이지번호를 클릭했을 때 전달되는 값
