document.addEventListener("DOMContentLoaded", () => {
  ...
  // 문자, 숫자, 특수문자가 포함된 8자리~ 150자리까지 문자열 검사패턴
  const passRule2 =
    /^.*(?=^.{8,15}$)(?=.*\d)(?=.*[a-zA-Z])(?=.*[!@#$%^&+=]).*$/;
  ...
  /*
  input box 에서 focus가 벗어났을 때 발생하는 event
  focusout, blur
  ...
  */
  ...
  password?.addEventListener("input", (e) => {
    const current = e.currentTarget;
    const error = error.divs[div_index.password];
  ...
  re_password?.addEventListener("input", (e) => {
    ...
    if (current.value === password?.value) {
      ...
      error_msg(error, " * 다시 확인해주세요");
    }
  }); // end re_password

  email?.addEventListener("input", (e) => {
    const current = e.currentTarget;
    const error = error.divs[div_index.email];
