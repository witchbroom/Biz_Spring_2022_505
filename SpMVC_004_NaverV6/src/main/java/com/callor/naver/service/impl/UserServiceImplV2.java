 ...
 * UserService 인터페이스를 사용하여 bean 주입받겠다 라고 Controller에 선언을 했다
 * 
 * 1. 생성자를 통해서 주입받기
 * 2. Autowired 를 사용하여 setter 를 통하여 주입받기
 * 
 * 어디에도 해당하는 bean 을 생성, 선언한 곳이 없을 때 발생하는 오류
 */
@Slf4j
...
public class UserServiceImplV2 extends UserServiceImplV1 {
	...
	public int join(UserVO userVO) {
		...
		String endPassword = passwordEncoder.encode(planPassword);
		
		// 암호화된 password 를 다시 VO 에 setting
		userVO.setPassword(endPassword);
	...
	public UserVO login(UserVO userVO) {
		...
		// (평문, 암호화된 문자열)
		if(passwordEncoder.matches(encPassword, planPassword)) {
			return loginUser;
		}
		return null;		
	}
