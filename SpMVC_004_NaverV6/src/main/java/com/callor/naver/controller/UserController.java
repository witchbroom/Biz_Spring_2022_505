...
public class UserController {
	...
	// UserService interface 를 상속받아 어떤 클래스를 주입받겠다 라는 선언
	@Autowired
	 ...
	 * Controller의 각 method 는 Spring 으로부터 HttpSession 값을
	 * 주입받을 수 있다ㄴ
	 * 
	 ...
	 * Session 제거하지 않는 한 계속 메모리에 값이 남아 있다
	 * 
	 * 큰 데이터를 HttpSession 에 담는 것은 썩 좋지 않다
	 * 최소한으로 status 를 유지할 수 있는 데이터만 담는 것이 좋다
	...
	public String mypage(HttpSession session, Model model) {
		...
		// 로그인한 사용자의 도서구입 리스트 가져오기
		/*
		 * buyBooks 리스트에는 b_isbn, b_username, b_date 값만 담긴 상태이다
		 * 왜, tbl_books에 3개의 칼럼만 있기 때문에
		 * 이 데이터만 가지고는 구체적인 도서정보를 알 수 없다
		 */
